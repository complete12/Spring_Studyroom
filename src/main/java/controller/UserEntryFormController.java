...
public class UserEntryFormController {
	...
	@RequestMapping(value = "/userentryform/edituserinfo", method = RequestMethod.POST)
	public String editUserInfo(@ModelAttribute User user, HttpSession session){
		User loginUser = (User) session.getAttribute(WebConstants.USER_KEY);
		user.setUserId(loginUser.getUserId());
		this.shopService.editUserInfo(user);
		
		session.removeAttribute(WebConstants.USER_KEY);
		loginUser = this.shopService.getUserByUserIdAndPassword(user.getUserId(), user.getPassword());
